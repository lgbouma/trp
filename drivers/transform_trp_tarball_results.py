...
def build_pipeline_dataframe(tarballdir,
                             overwrite_bool=0):
    """
    ...
    if len(tarballpaths) > 0 and len(logpaths) == 0:
    ...
    if not os.path.exists(trimoutcsvpath) or overwrite_bool:
        colnames = (
...
if __name__ == "__main__":
    ...
    if os.path.exists(trimoutcsvpath):
        overwrite = input(
            "Do you want to overwrite the previously found file "
            f"{trimoutcsvpath}? (y/n): "
        ).lower().strip()
        overwrite_bool = overwrite == 'y'

    if not os.path.exists(trimoutcsvpath) or overwrite_bool:
        build_pipeline_dataframe(
            tarballdir, overwrite_bool=overwrite_bool
        )
